...
public class ThresholdStepcounter {
	...
	public static void main(String[] args){
		if(args.length == 6){
			ThresholdStepcounter s = new ThresholdStepcounter(args);}
		else{
			args = new String[]{"pos", "0", "pos", "0", "pos", "0"};
			ThresholdStepcounter s = new ThresholdStepcounter(args);
			}
	}

	ThresholdStepcounter(String[] args){
	...
	private LinkedList<Timestamp> detectSteps(File output) {
		...
		for(int i = BUFFER; i < valuesY.length - BUFFER; i++){
			...
			if(freezeFor == 0){
				...
				if((THRESHOLD_DIRECTION[2] && valuesZ[i] > THRESHOLD[2]) || (!THRESHOLD_DIRECTION[2] && valuesZ[i] < THRESHOLD[2])){
